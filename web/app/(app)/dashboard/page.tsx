  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <header className="mb-8">
        <h1 className="text-2xl font-semibold text-slate-100">Início</h1>
        <p className="text-sm text-slate-400">Visão geral do painel</p>
      </header>
      ...
      <section className="bg-slate-900 rounded-xl border border-slate-800 p-5">
        <h2 className="text-sm font-semibold text-slate-100 mb-3">Últimas mensagens</h2>
        {recent.length === 0 ? (
          ...
          <ul className="space-y-2">
            {recent.map((m) => (
              ...
              >
                <span className="truncate flex-1 text-slate-300">{m.content}</span>
                <span className="text-xs text-slate-500 ml-3">{m.status}</span>
              </li>
            ))}
          </ul>
        )}
...
function Stat({
  ...
  return (
    <Link
      href={href}
      className="bg-slate-900 rounded-xl border border-slate-800 p-5 hover:border-emerald-500/50 hover:bg-slate-800/50 transition"
    >
